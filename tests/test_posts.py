...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("some title", "some content", False),
        ("tallest buildings", "tall tower 1", True),
    ],
)
...
def test_create_post_default_publshed_true(authorized_client, test_user):
    res = authorized_client.post(
        "/posts/", json={"title": "bla bla bla", "content": "la la la"}
    )
    created_post = PostResponse(**res.json())
    assert res.status_code == 201
...
def test_delete_post_non_exist(authorized_client, test_user, test_posts):
    res = authorized_client.delete("/posts/1000065120")
    assert res.status_code == 404
...
def test_update_post(authorized_client, test_user, test_posts):
    ...
    updated_post = PostCreate(**res.json())
    assert res.status_code == 200
...
def test_update_other_user_post(authorized_client, test_user, test_user2, test_posts):
    data = {"title": "updated title", "content": "new content 123"}
