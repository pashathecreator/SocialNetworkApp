...
@pytest.fixture
def voted_post(session, test_user, test_posts):
    new_vote = Vote(user_id=test_user["id"], post_id=test_posts[3].id)
...
def test_vote_twice_on_post(authorized_client, voted_post):
    res = authorized_client.post("/vote/", json={"post_id": voted_post.id, "dir": 1})
